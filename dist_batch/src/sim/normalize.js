import { asNonNegInt, clampInt } from "./util.js";
export function normalizeState(state) {
    ...
    if (!anyFlags._tuning || typeof anyFlags._tuning !== "object")
        anyFlags._tuning = {};
    const t = anyFlags._tuning;
    const cv = t.court_variant;
    if (cv !== "A" && cv !== "B" && cv !== "C")
        t.court_variant = "B";
    if (!(typeof t.fertility_mult === "number" && Number.isFinite(t.fertility_mult)))
        t.fertility_mult = 2.0;
    if (!(typeof t.mortality_mult === "number" && Number.isFinite(t.mortality_mult)))
        t.mortality_mult = 0.8;
}
