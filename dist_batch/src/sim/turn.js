...
import { deepCopy, clampInt, asNonNegInt } from "./util.js";
import { TURN_YEARS, BUSHELS_PER_FARMER_PER_YEAR, BUSHELS_PER_PERSON_PER_YEAR, BASE_FERTILITY, SPOILAGE_RATE_BASE, SPOILAGE_RATE_GRANARY, MARKET_PRICE_MIN, MARKET_PRICE_MAX, SELL_CAP_FACTOR_MIN, SELL_CAP_FACTOR_MAX, BUILD_RATE_PER_BUILDER_PER_TURN, BUILDER_EXTRA_BUSHELS_PER_YEAR, UNREST_SHORTAGE_PENALTY, UNREST_ARREARS_PENALTY, UNREST_BASELINE_DECAY_WHEN_STABLE, EVENTS_PER_TURN_PROBS, maxLaborDeltaPerTurn, MORTALITY_MULT_WITH_PHYSICIAN, BIRTH_CHANCE_BY_FERTILITY, BIRTH_FERTILE_AGE_MIN, BIRTH_FERTILE_AGE_MAX, MORTALITY_P_UNDER16, MORTALITY_P_UNDER40, MORTALITY_P_UNDER55, MORTALITY_P_UNDER65, MORTALITY_P_65PLUS, YIELD_MULT_FIELD_ROTATION, YIELD_MULT_DRAINAGE_DITCHES, SELL_MULT_MILL_EFFICIENCY, DRAINAGE_WEATHER_SOFTEN_BONUS, VILLAGE_FEAST_UNREST_REDUCTION } from "./constants.js";
import { normalizeState } from "./normalize.js";
import { EVENT_DECK } from "../content/events.js";
import { IMPROVEMENTS, hasImprovement } from "../content/improvements.js";
...
import { computeTierSets } from "./tiers.js";
import { deriveHouseholdRoster } from "./householdView.js";
import { processNobleFertility } from "./demography.js";
import { listEligibleCandidates, reserveCandidate, clearReservation, gcExpiredReservations } from "./marriageMarket.js";
function modsObj(state) {
    const anyFlags = state.flags;
    if (!anyFlags._mods || typeof anyFlags._mods !== "object")
        anyFlags._mods = {};
    return anyFlags._mods;
}
function cooldownsObj(state) {
    const anyFlags = state.flags;
    if (!anyFlags._cooldowns || typeof anyFlags._cooldowns !== "object")
        anyFlags._cooldowns = {};
    return anyFlags._cooldowns;
}
function consumeMod(state, key, defaultValue = 1) {
...
function currentSpoilageRate(state) {
    if (hasImprovement(state.manor.improvements, "granary_upgrade"))
        return SPOILAGE_RATE_GRANARY;
    return SPOILAGE_RATE_BASE;
}
function stewardshipMultiplier(state) {
    const s = state.house.head.traits.stewardship;
    // small, legible
    return 1 + (s - 3) * 0.02; // L1=-0.04 ... L5=+0.04
}
function yieldMultiplier(state) {
    let m = 1.0;
    if (hasImprovement(state.manor.improvements, "field_rotation"))
        m *= YIELD_MULT_FIELD_ROTATION;
    if (hasImprovement(state.manor.improvements, "drainage_ditches"))
        m *= YIELD_MULT_DRAINAGE_DITCHES;
    return m;
}
function sellMultiplier(state) {
    let m = 1.0;
    if (hasImprovement(state.manor.improvements, "mill_efficiency"))
        m *= SELL_MULT_MILL_EFFICIENCY;
    return m;
}
...
function boundedSnapshot(state) {
    ...
    return deepCopy({
        ...
        people: state.people,
        houses: state.houses,
        player_house_id: state.player_house_id,
        kinship_edges: state.kinship_edges ?? state.kinship,
...
function computeWeatherMarket(state) {
...
function applyProductionAndConstruction(state, weather_multiplier) {
...
function applyObligationsAndArrearsPenalty(state, production_bushels) {
...
function relationshipDrift(state) {
...
function householdPhase(state, houseLog) {
    const births = [];
    const deaths = [];
    let popDelta = 0;
    ...
    // NOTE: We intentionally do not apply the player's Physician improvement to the world.
    {
        ...
        if (reg) {
            ...
            state.flags._world_deaths_last_turn = deathsThisTurn;
        }
    }
...
function buildMarriageWindow(state, tierSets) {
...
function clearMarriageReservationsByProspectId(state, prospectId) {
    const anyFlags = state.flags;
    const raw = anyFlags?.marriage_reservations;
...
function buildProspectsWindow_v0_2_3(state, marriageWindow, prospectsLog) {
    ...
    for (const ref of active0) {
        if (t > ref.expires_turn) {
            ...
            if (p && p.type === "marriage") {
                const spouseId = p.spouse_person_id;
                if (typeof spouseId === "string" && spouseId.length > 0)
                    clearReservation(state, spouseId);
            }
            if (p) {
                prospectsLog.push({
...
function applyProspectsDecision(state, ctx, decisions, prospectsLog) {
    ...
    for (const a of actions) {
        ...
        if (prospect.type === "marriage") {
            const spouseForReservation = prospect.spouse_person_id;
...
function applyEvents(state) {
    ...
    for (const def of EVENT_DECK) {
        ...
        if (weight > 0)
            eligible.push({ def, weight, notes });
    }
    ...
    for (let i = 0; i < k && local.length > 0; i++) {
        ...
        const deltas = Object.keys(before).map((k2) => ({
            key: k2,
            before: before[k2],
            after: after[k2],
...
function computeTopDrivers(report, before, after) {
    const drivers = [];
...
export function proposeTurn(state) {
    ...
    const anyWorking = working;
    const playerHouseIdPF = typeof anyWorking.player_house_id === "string" ? anyWorking.player_house_id : "h_player";
    const houseLog = [];
    ...
    // 5.5) noble demography (Tier0/1 only; excludes the player House to avoid double-processing with householdPhase).
    {
        ...
        const demog = processNobleFertility(working, { tier0_house_ids: tier0HouseIds, tier1_house_ids: tier1HouseIds }, { float01: (label) => demogRng.fork(label).next() }, { year: working.turn_index * TURN_YEARS });
        // Optional (debug-only) counter in flags; does not affect mechanics.
        if (demog && Array.isArray(demog.births)) {
            working.flags._demography_births_last_turn = demog.births.length;
        }
    }
    ...
    const derived = deriveHouseholdRoster(working, playerHouseIdPF);
    ...
    let widowedPersonId = null;
    if (spouse) {
        if (working.house.head.alive && !spouse.alive)
            widowedPersonId = working.house.head.id;
        else if (!working.house.head.alive && spouse.alive)
            widowedPersonId = spouse.id;
    }
    const peoplePF = working.people && typeof working.people === "object" ? working.people : {};
    const rosterView = {
        ...
        rows: derived.map((r) => {
            ...
            if (!alive)
                badges.push("deceased");
            if (alive && widowedPersonId === r.person_id && sex)
                badges.push(sex === "M" ? "widower" : "widow");
            if (heirId && r.person_id === heirId)
                badges.push("heir");
...
function payBushels(state, amount) {
...
function applyObligationPaymentsAndPenalties(state, decisions, reportNotes) {
    const ob = state.manor.obligations;
    // Pay arrears first
...
function applyConstructionDecision(state, decisions, reportNotes) {
...
function applyMarriageDecision(state, ctx, decisions, reportNotes) {
    ...
    if (d.action === "accept") {
        ...
        // Apply dowry
        if (dowry >= 0)
            state.manor.coin = asNonNegInt(state.manor.coin + dowry);
        else
        ...
        const playerHouseId = String(state.player_house_id ?? "h_player");
...
function closeTurn(state, reportNotes, houseLog) {
    ...
    if (ob.tax_due_coin > 0)
        ob.arrears.coin = asNonNegInt(ob.arrears.coin + ob.tax_due_coin);
    if (ob.tithe_due_bushels > 0)
        ob.arrears.bushels = asNonNegInt(ob.arrears.bushels + ob.tithe_due_bushels);
    ob.tax_due_coin = 0;
    ...
    const shortage = Boolean(state.flags.Shortage);
...
export function applyDecisions(state, decisions) {
    ...
    const base = needsMigration ? deepCopy(state) : state;
    if (needsMigration)
        ensurePeopleFirst(base);
    ...
    const cleanedPriorLog = (base.log ?? []).map((e) => {
        const sb = e.snapshot_before;
        const sa = e.snapshot_after;
        const cleanBefore = sb && typeof sb === "object" && "log" in sb ? boundedSnapshot(sb) : sb;
        const cleanAfter = sa && typeof sa === "object" && "log" in sa ? boundedSnapshot(sa) : sa;
    ...
    const prospectsLog = [...(ctx.report.prospects_log ?? [])];
    // 10) apply decisions
    ...
    for (const e of ctx.report.house_log ?? [])
        lifeLog.push(e);
    ...
    const orderedHouseLog = (lifeLog ?? []).map((e, i) => ({ e, i })).sort((a, b) => {
        if (a.e.turn_index !== b.e.turn_index)
            return a.i - b.i;
        const w = (k) => (k === "succession" ? 0 : k === "widowed" ? 1 : 2);
        const da = w(a.e.kind);
        const db = w(b.e.kind);
        if (da !== db)
            return da - db;
