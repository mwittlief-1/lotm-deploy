import { deriveCourtMemberIds } from "./court.js";
/**
...
function getTuningNumber(state, key) {
...
function indexPersonToHouseId(houses) {
    const out = new Map();
    for (const hid of Object.keys(houses).sort()) {
        ...
        if (!h || typeof h !== "object")
            continue;
        const candidates = [];
        if (typeof h.head_id === "string" && h.head_id)
            candidates.push(h.head_id);
        if (typeof h.spouse_id === "string" && h.spouse_id)
            candidates.push(h.spouse_id);
        if (Array.isArray(h.child_ids)) {
            for (const x of h.child_ids)
        ...
        for (const pid of candidates) {
            ...
            if (!cur || hid.localeCompare(cur) < 0)
                out.set(pid, hid);
        }
    }
    return out;
}
function addPerson(set, personId) {
    if (typeof personId === "string" && personId.length > 0)
        set.people.add(personId);
}
function addHouse(set, houseId) {
    if (typeof houseId === "string" && houseId.length > 0)
        set.houses.add(houseId);
}
function addInstitution(set, instId) {
    if (typeof instId === "string" && instId.length > 0)
        set.institutions.add(instId);
}
function addHouseForPerson(set, personToHouse, personId) {
    if (typeof personId !== "string" || personId.length === 0)
        return;
...
function readIdList(maybe) {
    ...
    for (const x of maybe) {
        if (typeof x === "string" && x.length > 0)
            out.push(x);
        else if (x && typeof x === "object" && typeof x.id === "string" && x.id.length > 0)
            out.push(x.id);
    }
    return out;
}
function selectInstitutionsByType(institutions, types, cap) {
 ...
 */
export function computeTierSets(state) {
    ...
    for (const pid of deriveCourtMemberIds(state))
        addPerson(tier0, pid);
    ...
    for (const id of readIdList(anyState.locals?.liege_chain))
        addPerson(tier0, id);
    ...
    const parishId = (typeof anyState.locals?.parish_institution_id === "string" && anyState.locals.parish_institution_id) ||
        (typeof anyState.manor?.parish_institution_id === "string" && anyState.manor.parish_institution_id) ||
        null;
    if (parishId && institutions[parishId])
        addInstitution(tier0, parishId);
    ...
    if (!parishId) {
        const parishCandidates = Object.keys(institutions)
            .filter((id) => {
            const inst = institutions[id];
            if (!inst || typeof inst !== "object" || inst.type !== "parish")
                return false;
            const patron = inst.patron_actor_id;
            if (patron && typeof patron === "object" && patron.kind === "house" && patron.id === playerHouseId)
                return true;
            return inst.patron_house_id === playerHouseId;
        })
            .sort();
        if (parishCandidates.length > 0)
            addInstitution(tier0, parishCandidates[0]);
    }
    // Map Tier0 persons to their houses (when known in the registry).
    for (const pid of tier0.people)
        addHouseForPerson(tier0, personToHouse, pid);
    ...
    const priorityHouses = new Set();
    for (const pid of tier0.people) {
        const hid = personToHouse.get(pid);
        if (hid && hid !== playerHouseId)
            priorityHouses.add(hid);
    }
    ...
    const addTier1House = (hid) => {
        if (!hid)
            return;
        if (tier0.houses.has(hid))
            return;
        if (tier1.houses.has(hid))
            return;
        if (tier1.houses.size >= cap)
            return;
        tier1.houses.add(hid);
    };
    for (const hid of [...priorityHouses].sort())
        addTier1House(hid);
    for (const hid of Object.keys(houses).sort())
        addTier1House(hid);
    ...
    for (const iid of selectInstitutionsByType(institutions, ["bishopric", "abbey"], instCap))
        addInstitution(tier1, iid);
    // --- Tier2 (stubs) ---
    for (const hid of Object.keys(houses).sort()) {
        if (tier0.houses.has(hid))
            continue;
    ...
    for (const iid of Object.keys(institutions).sort()) {
        if (tier0.institutions.has(iid))
            continue;
        if (tier1.institutions.has(iid))
            continue;
