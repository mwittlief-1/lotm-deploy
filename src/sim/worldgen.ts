...
function extPersonId(i: number, role: string): string {
...
function ensureRelationshipEdgesToPlayerHead(state: RunState, extHouseIds: string[]): void {
  ...
  for (const hid of [...extHouseIds].sort()) {
    const h = houses?.[hid];
    const headId: string =
      h && typeof h === "object" && typeof h.head_id === "string" && h.head_id
        ? h.head_id
...
function childCountForTier(tier: HouseTier, rng: Rng): number {
  ...
  if (tier === "Knight") return rng.int(1, 3);
  if (tier === "Baron") return rng.int(2, 4);
  return rng.int(3, 5);
}
...
function smoothExistingChildrenAgesInPlace(opts: {
  ...
  const kids = childIds
    .map((id) => people[id])
    .filter((p) => p && typeof p === "object" && typeof p.age === "number") as Array<{ id: string; age: number }>;
...
function smoothPlayerHouseholdChildren(state: RunState): void {
  const anyState: any = state as any;
  const people = anyState.people as Record<string, any>;
  ...
  const anyFlags: any = anyState.flags;
  const mark = (flag: string) => {
    if (!anyFlags || typeof anyFlags !== "object") return;
    if (!anyFlags._worldgen_notes || typeof anyFlags._worldgen_notes !== "object") anyFlags._worldgen_notes = {};
...
function isTuningBool(state: RunState, key: string): boolean {
...
function ensureFamilySnapshotForHouse(opts: {
  ...
  const tier: HouseTier = (prior.tier as HouseTier) ?? (hRng.pick(HOUSE_TIERS) as HouseTier);
  const surname: string = (typeof prior.name === "string" && prior.name) ? prior.name : hRng.pick(HOUSE_NAMES);
  const holdings_count: number = typeof prior.holdings_count === "number" ? prior.holdings_count : holdingsForTier(tier, hRng.fork("holdings"));
  ...
  const priorChildIds: string[] = Array.isArray(prior.child_ids) ? prior.child_ids.filter((x: any) => typeof x === "string" && x.length > 0) : [];
  const childIds: string[] = [...priorChildIds];
 ...
 */
export function ensureExternalHousesSeed_v0_2_8(state: RunState): void {
  const s: any = state as any;
  ...
  const institutions = s.institutions as Record<string, any>;

  const anyLocals: any = (state as any).locals;
  const anyManor: any = (state as any).manor;
  const existingParishId: any = (anyLocals && anyLocals.parish_institution_id) || (anyManor && anyManor.parish_institution_id) || null;
  const isValidParishId = (pid: any): pid is string => {
    if (typeof pid !== "string" || !pid) return false;
    const inst: any = institutions[pid];
    return Boolean(inst && typeof inst === "object" && inst.type === "parish");
  };

  let parishId: string | null = isValidParishId(existingParishId) ? String(existingParishId) : null;
  if (!parishId) {
    parishId = "i_parish_player_local";
    const cur: any = institutions[parishId];
  ...
  if (anyLocals && !isValidParishId(anyLocals.parish_institution_id)) anyLocals.parish_institution_id = parishId;
  if (anyManor && !isValidParishId(anyManor.parish_institution_id)) anyManor.parish_institution_id = parishId;
  ...
  const people = s.people as Record<string, any>;
  const houses = s.houses as Record<string, any>;
