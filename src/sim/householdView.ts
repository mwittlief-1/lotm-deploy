...
export type HouseholdRole = "head" | "spouse" | "child" | "sibling" | "parent";
...
function hasPerson(state: AnyRecord, person_id: string): boolean {
  if (!person_id) return false;
  const p = getPeopleMap(state)?.[person_id];
  return Boolean(p && typeof p === "object");
}

function getAge(state: AnyRecord, person_id: string): number | null {
  const p = getPeopleMap(state)?.[person_id];
  if (!p) return null;
...
function sortRoster(state: AnyRecord, rows: HouseholdRosterRow[]): HouseholdRosterRow[] {
...
export function deriveHouseholdRoster(state: AnyRecord, house_id: string): HouseholdRosterRow[] {
  ...
  if (!hasPerson(state, headId)) return [];
  if (!isAlive(state, headId)) return [];

  // Assign roles in priority order; keep first assignment if duplicates.
  const roleById = new Map<string, HouseholdRole>();
  const add = (person_id: string, role: HouseholdRole) => {
    if (!person_id) return;
    if (!hasPerson(state, person_id)) return;
    if (!roleById.has(person_id)) roleById.set(person_id, role);
  };
  ...
  for (const pid of getParents(state, headId)) add(pid, "parent");
  for (const sid of getSiblings(state, headId)) add(sid, "sibling");
  for (const cid of getChildren(state, headId)) add(cid, "child");
