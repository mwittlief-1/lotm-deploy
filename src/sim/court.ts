...
export function buildCourtRoster_v0_2_4(state: RunState, houseLog?: HouseLogEvent[]): CourtRoster {
  ...
  const houseRec: any = getHouseRegistry(state);
  const memberIdsRaw: any[] = Array.isArray(houseRec?.member_person_ids) ? houseRec.member_person_ids : [];
  const memberIds = memberIdsRaw.filter((x) => typeof x === "string" && x.length > 0).sort((a, b) => String(a).localeCompare(String(b)));
  for (const id of memberIds) pushRow(String(id), "resident");
...
export function courtConsumptionBushels_v0_2_4(state: RunState, bushelsPerPersonPerYear: number, turnYears: number, houseLog?: HouseLogEvent[]): {
  ...
  for (const r of roster.rows) {
    ...
    const w = consumptionWeightByAge(age);
    adultEq += w;
  ...
  return {
    ...
    court_consumption_breakdown: {
      ...
      adults_total_bushels: adultsTotal,
      children_count: childrenCount,
      children_total_bushels: childrenTotal,
...
function consumptionWeightByAge(ageYears: number): number {
