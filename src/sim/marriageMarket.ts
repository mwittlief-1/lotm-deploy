...
export type AgeBand = { min: number; max: number };

export type MarriageMarketScope =
  | { kind: "default" }
  | { kind: "all" }
  | { kind: "house_ids"; house_ids: string[] }
  | { kind: "prefer_prefix"; prefix: string; fallback_to_all?: boolean };

export type ListEligibleCandidatesParams = {
  subject_person_id: PersonId;
  scope?: MarriageMarketScope;
  ageBands?: AgeBand[];
};

export type ReservationEntry = { prospect_id: string; expires_turn: number };
export type ReservationMap = Record<PersonId, ReservationEntry>;

const DEFAULT_AGE_BANDS: AgeBand[] = [{ min: 15, max: 45 }];

// --- Reservations ---

function readReservations(state: RunState): ReservationMap {
  const raw: any = (state as any)?.flags?.marriage_reservations;
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
  return raw as ReservationMap;
}

function ensureReservations(state: RunState): ReservationMap {
  ...
  return f.marriage_reservations as ReservationMap;
}

export function isReserved(state: RunState, person_id: PersonId, turn: number): boolean {
  if (!person_id) return false;
  const m = readReservations(state);
  const e = m[person_id];
  if (!e || typeof e !== "object") return false;
  const ex = (e as any).expires_turn;
...
export function clearReservation(state: RunState, person_id: PersonId): void {
  if (!person_id) return;
  const m = ensureReservations(state);
  delete m[person_id];
}

export function gcExpiredReservations(state: RunState, current_turn: number): void {
  const m = ensureReservations(state);
  ...
  for (const pid of Object.keys(m).sort((a, b) => a.localeCompare(b))) {
    ...
    if (t > Math.trunc(ex)) delete m[pid];
  }
}

// --- Candidate query ---

function readRegistries(state: RunState): {
  ...
  const s: any = state as any;
  const people = s.people && typeof s.people === "object" ? (s.people as Record<string, Person>) : {};
  const houses = s.houses && typeof s.houses === "object" ? (s.houses as Record<string, any>) : {};
  const player_house_id = typeof s.player_house_id === "string" ? (s.player_house_id as string) : "h_player";
  const kinship_edges: KinshipEdge[] = Array.isArray(s.kinship_edges)
...
function ageInBands(age: number, bands: AgeBand[]): boolean {
...
function desiredSpouseSex(subject: Person | null): Sex | null {
  if (!subject) return null;
  return subject.sex === "M" ? "F" : subject.sex === "F" ? "M" : null;
}

function getSubjectPerson(state: RunState, people: Record<string, Person>, subjectId: string): Person | null {
  const p = people[subjectId];
  if (p && typeof p === "object") return p;
  // Legacy fallback: subject may only exist in embedded state.house/locals.
  if (state.house?.head?.id === subjectId) return state.house.head;
  if (state.house?.spouse?.id === subjectId) return state.house.spouse;
  for (const c of state.house?.children ?? []) {
    if (c?.id === subjectId) return c;
  }
  if (state.locals?.liege?.id === subjectId) return state.locals.liege;
  if (state.locals?.clergy?.id === subjectId) return state.locals.clergy;
...
function tierRankFromHouseTier(tier: unknown): number {
...
function tierProximityKey(h: any): { tier: number; prox: number } {
  const tierNum =
    numOrNull(h?.instantiation_tier) ??
...
const SPOUSE_A_KEYS = ["a_id", "from_person_id", "from", "a"] as const;
const SPOUSE_B_KEYS = ["b_id", "to_person_id", "to", "b"] as const;

function readIdByKeys(obj: any, keys: readonly string[]): string | null {
...
function spouseEndpoints(e: any): { a: string | null; b: string | null } {
  if (!e || typeof e !== "object" || e.kind !== "spouse_of") return { a: null, b: null };
  return {
...
function buildSpouseIndex(kinship_edges: KinshipEdge[]): {
  spouseIds: Map<string, Set<string>>;
  hasSpouseEdge: Set<string>;
} {
  const spouseIds = new Map<string, Set<string>>();
  const hasSpouseEdge = new Set<string>();

  for (const e of kinship_edges) {
    const { a, b } = spouseEndpoints(e as any);
    if (!a && !b) continue;
    if (a) hasSpouseEdge.add(a);
    if (b) hasSpouseEdge.add(b);

    if (a && b) {
      if (!spouseIds.has(a)) spouseIds.set(a, new Set());
      if (!spouseIds.has(b)) spouseIds.set(b, new Set());
...
function livingSpouseIdFromIndex(
  person_id: string,
  idx: { spouseIds: Map<string, Set<string>> },
  people: Record<string, Person>
): string | null {
  ...
  for (const sid of spouses) {
    const sp = people[sid];
    if (sp && typeof sp === "object" && sp.alive === true) return sid;
  }
  return null;
}

function resolveScopeHouseIds(
  houses: Record<string, any>,
  player_house_id: string,
  subject_house_id: string | null,
  scope: MarriageMarketScope | undefined
): string[] {
...
export function listEligibleCandidates(state: RunState, params: ListEligibleCandidatesParams): string[] {
  ...
  const bestByPid = new Map<string, { pid: string; hid: string; tier: number; prox: number }>();
  ...
  for (const hid of houseIds) {
    ...
    for (const pid of [...ids].sort((a, b) => a.localeCompare(b))) {
      ...
      if (person.married === true && !spouseIdx.hasSpouseEdge.has(pid)) continue;

      if (isReserved(state, pid, state.turn_index)) continue;
