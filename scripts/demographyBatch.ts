...
export type Summary = {
  ...
  seeds_surviving_past_turn_29_count: number | null;
  seeds_surviving_past_turn_29_share: number | null;
};
...
export function runDemographyBatch(
  ...
  for (const seed of seeds) {
    ...
    for (let t = 0; t < turns; t++) {
      ...
      for (const [pid, p] of Object.entries(next.people ?? {})) {
        ...
        if (mom) {
          const motherResidenceId =
            typeof (mom as any)?.residence_house_id === "string" && (mom as any).residence_house_id
              ? String((mom as any).residence_house_id)
              : (typeof (mom as any)?.house_id === "string" && (mom as any).house_id ? String((mom as any).house_id) : null);
          const playerHouseId = typeof (next as any).player_house_id === "string" ? String((next as any).player_house_id) : "h_player";
          if (motherResidenceId === null) birthsByMotherResidency.unknown_mother_or_residency += 1;
          else if (motherResidenceId === playerHouseId) birthsByMotherResidency.player_house_resident += 1;
  ...
  const canMeasurePast29 = turns >= 30;
  const survivePast29Count = canMeasurePast29 ? seedOutcomes.filter((row) => row.survived_past_turn_29).length : 0;
  ...
  return {
    ...
    seeds_surviving_past_turn_29_count: canMeasurePast29 ? survivePast29Count : null,
    seeds_surviving_past_turn_29_share: canMeasurePast29 && seedOutcomes.length > 0 ? Number((survivePast29Count / seedOutcomes.length).toFixed(6)) : null,
  };
}
