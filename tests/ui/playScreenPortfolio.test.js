...
import { buildPortfolioEvidenceScope, buildPortfolioMapCheckpoint, buildPortfolioOverviewSurface, buildPortfolioScopeContract, selectPortfolioManor } from "../../src/ui/playScreenPortfolio";
const PREVIEW_STATE = {
    world_topology_view: {
...
describe("playScreenPortfolio", () => {
    ...
    it("resolves selected manor detail and evidence scope from the shared contract", () => {
        ...
        expect(buildPortfolioEvidenceScope({
            contract,
            scopeMode: "portfolio",
        ...
        expect(buildPortfolioEvidenceScope({
            ...
            chipHelperText: "Hx 30001 detail is selected above, but the headline chips still follow the current manor ledger because only the home manor has a resolved turn ledger right now.",
            diffLedgerHelper: "Hx 30001 detail is selected above, but this ledger still follows the current manor because non-anchor holdings do not yet expose their own resolved turn ledger.",
            diffLedgerScopeLabel: "Current manor ledger · Hx 30001 selected",
            receiptScopeLabel: "Hx 30001 selected",
            receiptScopeSummary: "Hx 30001 detail is selected in Holdings, but Explain Changes still follows the current manor ledger. Use the selector for holdings comparison without assuming a second resolved ledger.",
    ...
    it("builds a stable map checkpoint affordance without assuming a live map host", () => {
        ...
        expect(buildPortfolioMapCheckpoint({
            contract,
        ...
        expect(buildPortfolioMapCheckpoint({
            ...
            buttonLabel: "Center on selected holding",
            helper: "Hx 30001 is selected in Holdings, but this bounded snapshot still only exposes a concrete holding target for the current manor. The control stays dormant until selected-holding targets widen beyond the anchor manor.",
            state: "dormant",
            statusLabel: "Dormant",
            target: {
                countyId: null,
    ...
    it("keeps the overview surface projection stable while the scope contract grows", () => {
        ...
        expect(surface?.summaryCards).toEqual(expect.arrayContaining([
