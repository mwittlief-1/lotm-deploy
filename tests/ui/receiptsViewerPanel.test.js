...
const STRUCTURED_PHASE_RESULTS = [
    {
        phase: "obligations",
...
describe("ReceiptsViewerPanel", () => {
    it("renders counterparty-first relationship lever cards in grouped mode", () => {
        ...
        const markup = renderToStaticMarkup(_jsx(ReceiptsViewerPanel, { counterpartySections: data.counterpartySections, groupedSections: data.groupedSections, mode: "grouped", onModeChange: () => undefined, rawPhases: data.rawPhases, scopeLabel: "Current manor ledger", scopeSummary: "Explain Changes is still following the current manor ledger. Holdings totals above remain summary context only." }));
    ...
    it("renders structured fiscal receipt fields in raw mode when present", () => {
        ...
        const markup = renderToStaticMarkup(_jsx(ReceiptsViewerPanel, { counterpartySections: data.counterpartySections, groupedSections: data.groupedSections, mode: "raw", onModeChange: () => undefined, rawPhases: data.rawPhases }));
